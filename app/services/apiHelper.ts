...
export const apiService = {
  ...
  update: <T = any, U = any>(resource: string, id: string, data: T) => apiClient.patch<U>(`/api/${resource}/update/${id}`, data),
  ...
  updateMultipart: <T = any, U = any>(resource: string, id: string, formData: FormData) =>
    apiClient.patch<U>(`/api/${resource}/update/${id}`, formData, { headers: { "Content-Type": "multipart/form-data" } }),
